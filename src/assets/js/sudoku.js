(() => {
    const config = JSON.parse(localStorage.getItem('sudoku-config'));
    ...
    valuePicker.addEventListener("click", ({target}) => {
        if (selectItem[0] === undefined ||
            selectItem[1] === undefined ||
            target.tagName !== 'SPAN') {
            return;
        }
        ...
        const value = target.innerHTML === '清除' ? '' : target.innerHTML;
        const index = selectItem[0] * 9 + selectItem[1];
        const previousVal = cellElems[index].innerHTML;
        ...
        if (value) {
            ...
            blockContainer.style.cssText = '';
            list[selectItem[0]][selectItem[1]].value = '';
        }
        cellElems[index].style.backgroundColor = `var(--bg${value})`;
        cellElems[index].innerHTML = value;
        const entries = Object.entries(errors);
        entries.forEach(([k, v]) => {
            if (k.includes(`${selectItem[0]},${selectItem[1]}`) && v === previousVal) {
                const pos = k.split("|").map(it => it.split(",").map(Number));
                cellElems[pos[0][0] * 9 + pos[0][1]].classList.remove('error');
                cellElems[pos[1][0] * 9 + pos[1][1]].classList.remove('error');
                delete errors[k];
            }
        })
        const err = check(selectItem[0], selectItem[1], Number(value));
        err.forEach(item => {
            errors[genKey(item[0], item[1])] = value;
        })
    ...
    window.addEventListener("beforeunload", () => {
        if (blankCount !== 0 || Object.keys(errors).length !== 0) {
            localStorage.setItem('sudoku-config', JSON.stringify({
                list,
                blankCount,
                errors
            }))
    ...
    function startNewGame(config) {
        ...
        list.forEach((item, i) => {
            item.forEach(({value}, j) => {
                if (errorsKey.some(item => item.includes(`${i},${j}`))) {
                    if (!cellElems[i * 9 + j].classList.contains('error')) {
                        cellElems[i * 9 + j].classList.add('error');
                    }
                } else {
    ...
    function check(x, y, value) {
        const error = [];
        for (let i = 0; i < 9; i++) {
            if (i !== y && list[x][i].value === value) {
                error.push([[x, i], [x, y]]);
            }
            if (i !== x && list[i][y].value === value) {
                error.push([[i, y], [x, y]]);
            }
        }
        ...
        for (let i = 0; i < 3; i++) {
            for (let j = 0; j < 3; j++) {
                if (baseRow + i !== x && baseCol + j !== y && list[baseRow + i][baseCol + j].value === value) {
                    error.push([[baseRow + i, baseCol + j], [x, y]]);
                }
            }
        }
        return error;
